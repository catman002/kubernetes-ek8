...
func Create_etcd_server(etcdServers map[string]string) {
	if !(common.ModFlags["etcdServerCERT"] == 1 ||
		common.ModFlags["caCONFIG"] == 1 ||
		common.ModFlags["etcdCA"] == 1) {
		return
	}
	common.TPrintln("创建kube-etcd证书 [server]...")
	var once sync.Once
	for _, ip := range etcdServers {
		...
		//创建证书
		create_etcd_cert(&once, ip, etcdServers, allca_dir, etcd_dir, etcd_dir_name, etcd_dir_tmp)
	}
...
func create_etcd_cert(once *sync.Once, ip string, etcdServers map[string]string,
	allca_dir string,
	etcd_dir string,
	...
	etcd_server_csr_file_name := "etcd-server-csr.json"
	etcd_server_csr_file := etcd_dir_tmp + "/" + etcd_server_csr_file_name

	template_csr_json_file := "factory/template/cert/etcd/" + etcd_server_csr_file_name
	CN := config.Cfg["etcd_CN"]

	createCert := func() {
		//创建etcd-server的证书请求的json配置文件
		new(common.FileAssist).
			Println("创建 "+etcd_server_csr_file_name+".....", "local").
			Load(template_csr_json_file).
			Set("etcd_CN", config.Cfg["etcd_CN"]).
			SaveAs(etcd_server_csr_file).Close()
		//创建etcd-server的证书请求、证书、私锁
		var etcd_hostname_list string
		for nodeName, _ip := range etcdServers {
			etcd_hostname_list = etcd_hostname_list + _ip + "," + nodeName + ","
		}
		...
		cmd := new(common.Command).Add("source config.cfg").
			Add("cfssl gencert -ca=" + etcd_ca_file + " -ca-key=" + etcd_key_file + " -config=" + ca_config_file + " -hostname=" + etcd_hostname_list + " -profile=server -loglevel=1 " + etcd_server_csr_file + " | cfssljson -bare " + etcd_dir_tmp + "/" + CN).
			Add(echo).
			ToString()
